...
const Navbar = () => {
  ...
  const activeStyle = "bg-[#37353E] text-[#D4D9D7] px-4 py-2 rounded-md transition-all";
  const inactiveStyle = "text-[#37353E] px-4 py-2 hover:opacity-70 transition-all";
    ...
    <div className="sticky top-0 z-50 w-full border-b border-black/5 bg-[#D4D9D7]/90 backdrop-blur-md  shadow-sm px-4">
      <nav className="flex justify-between items-center py-3 max-w-7xl mx-auto w-full">
        ...
        <ul className="hidden md:flex items-center text-[#695D5E] gap-8 text-sm font-medium">
          <li className={`${pathname === "/" ? "text-[#37353E] font-bold" : ""} hover:text-[#37353E] transition-colors`}>
            <Link href={"/"}>Home</Link>
          </li>
          <li className={`${pathname === "/all-books" ? "text-[#37353E] font-bold" : ""} hover:text-[#37353E] transition-colors`}>
            <Link href={"/all-books"}>All Books</Link>
          </li>
          <li className={`${pathname === "/profile" ? "text-[#37353E] font-bold" : ""} hover:text-[#37353E] transition-colors`}>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full right-0 bg-[#D4D9D7] border-b border-black/5 p-5 shadow-xl">
          <ul className="flex flex-col gap-4 text-[#695D5E] font-medium">
            ...
            <li><Link href={"/all-books"} onClick={() => setIsOpen(false)}>All Books</Link></li>
            <li><Link href={"/profile"} onClick={() => setIsOpen(false)}>My Profile</Link></li>
            <hr className="border-black/5" />
            <div className="flex flex-col gap-2">
               <Link 
                href={"/signup"} 
                onClick={() => setIsOpen(false)} 
                className={pathname === "/signup" ? "bg-[#37353E] text-[#D4D9D7] p-2 rounded" : "p-3"}
               >
                SignUp
               </Link>
               <Link 
                href={"/signin"} 
                onClick={() => setIsOpen(false)} 
                className={pathname === "/signin" ? "bg-[#37353E] text-[#D4D9D7] p-2 rounded" : "p-3"}
               >
                SignIn
               </Link>
            </div>
